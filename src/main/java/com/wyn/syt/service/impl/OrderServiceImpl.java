...
import com.wyn.syt.mapper.OrderMapper;
...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, Orders> implements OrderService {
    ...
    public void submit(Orders orders) {
        ...
        if(addressBook == null){
            throw new CustomException("用户位置信息有误，不能下单");
        }
        ...
        List<OrderDetail> orderDetails = shoppingCarts.stream().map((item) -> {
            ...
            orderDetail.setSort(50);
            orderDetail.setUserId(item.getUserId());
            amount.addAndGet(item.getAmount().multiply(new BigDecimal(item.getNumber())).intValue());

            Dish dish = dishService.getById(item.getDishId());
            Integer sort = dish.getSort();
            dish.setSort(sort + 5);
            /*Integer saleNum = dish.getSaleNum();
            dish.setSaleNum(saleNum + 1);*/
